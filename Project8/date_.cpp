...
int daysBetweenDates2(const date_ & a, const date_ & b)
{
	int counDays = 0;
...
int daysBetweenDates(const date_ & a, const date_ & b)
{
	...
	if (b.getYear() > a.getYear()) {
		...
		if (m == 1 || m == 3 || m == 5 || m == 7 || m == 8 || m == 10 || m == 12)
			counDays += 31 - b.getDay();
		if (m == 4 || m == 6 || m == 9 || m == 11)
			counDays += 30 - b.getDay();
		if (m == 2) {
			if (b.getYear() % 400 == 0 || (b.getYear() % 4 == 0 && b.getYear() % 100 != 0))
				counDays += 29 - b.getDay();
			else
				counDays += 28 - b.getDay();
		}
